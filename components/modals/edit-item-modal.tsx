...
export function EditItemModal({ item, open, onClose, onSuccess }: EditItemModalProps) {
  ...
  useEffect(() => {
    if (item) {
      const dateFound = typeof item.dateFound === 'string' 
    ...
    try {
      const res = await fetch(`/api/items/${item.id}`, {
        ...
        body: JSON.stringify({
          ...
          locationFound: formData.location === 'Room' 
            ? `Room ${formData.roomNumber}` 
            : formData.location,
          roomNumber: formData.location === 'Room' ? formData.roomNumber : undefined,
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field>
                ...
                <Select 
                  ...
                  onValueChange={(value) => setFormData({ ...formData, location: value as ItemLocation })}
                  required
                >
                  <SelectTrigger>
