...
function formatItemsForExport(items: LostItem[]): ExportRow[] {
  return items.map((item) => ({
    ...
    "Handover Date": item.handoverDate
      ? format(new Date(item.handoverDate), "MMM dd, yyyy HH:mm")
      : "",
    "Handed Over By": item.handoverBy || "",
    "Contact Number": item.handoverContactNumber || "",
    "Dispatch Date": item.dispatchDate
      ? format(new Date(item.dispatchDate), "MMM dd, yyyy HH:mm")
      : "",
...
export function exportToExcel(
  items: LostItem[],
  filename: string = "lost-and-found",
): void {
  ...
  const columnWidths = Object.keys(data[0] || {}).map((key) => ({
    wch: Math.max(
      key.length,
      ...data.map((row) => String(row[key as keyof ExportRow]).length),
    ),
  }));
